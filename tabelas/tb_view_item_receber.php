<?php
    ...
    where r.requisicao = $req AND i.status = 565";
    $sql = mysqli_query( $link, $sql );
    $painel = mysqli_fetch_array( $sql );
    $_SESSION['unidade_id_receber'] = $painel['id_unidade'];
?>
...
<table class="table table-sm table-striped dataTable js-exportable table-bordered">
    <thead>
        <tr class="text-center BG-INFO elevation-1">
            ...
            <th scope="col">QTD.ENV</th>
            <th scope="col">QTD.ENT</th>
    ...
    <tbody>
      ...
      INNER JOIN status s
      ON e.status = s.id
      where r.requisicao = $req AND e.status = 565";

      $sql = mysqli_query( $link, $sql );
        while ( $req = mysqli_fetch_array( $sql ) ){
            $_SESSION['item_id_receber'][$req['id_item_req']] = $req['itemId'];
    ?>
            <input type="hidden" name="id[]" value="<?php echo$req['id_item_req']; ?>">
            <tr class="text-center">
                <th scope="row"> <small> <?php echo$req['itemId']; ?> </th>
                <td><?php echo $nomeItem->nomeItemParametrizado($req['itemId']); ?></td>
                <td><?php echo $req['solicitado']; ?> </td>
                <td><?php echo $req['enviado']; ?> </td>
                <td ><input  type="number" name="qtd[]" value="<?php echo$req['enviado']; ?>"></td>
...
<button type="button" class="btn btn-xs btn-success col-12 elevation-3" id="salvar_recebimento"><strong>Confirmar Recebimento</strong></button>
<br>
    ...
    <h5><strong></h5>
    <p>
                No momento não existe item para recebimento!
    </p>
