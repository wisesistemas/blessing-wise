...
$sql = "SELECT  a.id as 'id_extra',  REPLACE( a.data, '/', '-') as 'data', b.nome as 'funcionario', c.nome as 'unidade', d.funcao as 'funcao', 
e.escala as 'escala', e.tipo as 'tipo', a.status as 'status', 
a.substituido as substituido, a.motivo as motivo, a.valor_pagar as valor, s.status as status, 
a.hora_ent as entrada, a.hora_sai as saida
FROM `extras` a
            ...
            ?>
                <tr class="extra text-center text-ms" id="<?php echo $extras["id_extra"] ?>">
                    ...
                    <td><?php
                                ...
                                ?></td>
                    <td><?php echo strtoupper($extras["motivo"]); ?></td>
                    <td><?php 
                    
                    if($extras["valor"] != ""){
                        echo 'R$: '.number_format($extras["valor"], 2, '.', '');
                    }else{
                        echo '';
                    }
             
                    ?></td>
                    <td><?php echo $extras["status"]; ?></td>
                </tr>
            <?php
            }
            ?>
